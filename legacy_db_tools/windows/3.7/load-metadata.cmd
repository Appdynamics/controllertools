...
echo Creating EJB Timer Table
echo Loading metadata...
if exist "%DESTINATION%\ejb__timer__tbl.sql" (
    "%CONTROLLER_HOME%\db\bin\mysql" %MYSQL_OPTS% controller < "%DESTINATION%\ejb__timer__tbl.sql" 2>>"%LOG%"
    if errorlevel 1 (
        echo Loading metadata failed!
        echo See: %LOG%
        exit /b 1
    ) else (
        move "%DESTINATION%\ejb__timer__tbl.sql" "%DESTINATION%\ejb__timer__tbl.done" >nul
    )
)