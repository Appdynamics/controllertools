...
set FOUND_PARTITIONS_TO_LOAD=false
pushd "%DESTINATION%"
for /f "delims=" %%t in ('dir /b *.sql 2^>nul') do (
    set DUMP_FILE=%%t

    if exist "!DUMP_FILE!" (
        if "!FOUND_PARTITIONS_TO_LOAD!" == "false" (
            set FOUND_PARTITIONS_TO_LOAD=true
        ...
        echo     Loading !TABLE_NAME!... 1>>"%LOG%"

        move "!DUMP_FILE!" "!TABLE_NAME!.loading" >nul
        if errorlevel 0 (
            if not exist "%DESTINATION%\!TABLE_NAME!.done" (
                "%CONTROLLER_HOME%\db\bin\mysql" %MYSQL_OPTS% controller < "!TABLE_NAME!.loading" 2>>"%LOG%"
                if errorlevel 1 (
                    echo Loading !TABLE_NAME! failed!
                    echo See: %LOG%
                ) else (
                    move "!TABLE_NAME!.loading" "!TABLE_NAME!.done" >nul
                )
            )
        )
    )
