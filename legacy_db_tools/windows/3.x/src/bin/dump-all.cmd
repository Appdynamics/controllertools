...
for /f "usebackq tokens=*" %%t in ("%DESTINATION%\types\partition-tables.txt") do (
    ...
    if exist "%DESTINATION%\!TABLE_NAME!.tmp" (
                        echo     partition table: ^(detected dump in progress^)   !TABLE_NAME! >> "%LOG%"
                        echo     partition table: ^(detected dump in progress^)   !TABLE_NAME!
    ) else (
        if exist "%DESTINATION%\!TABLE_NAME!.sql" (
                        echo     partition table: ^(detected completed dump^)     !TABLE_NAME! >> "%LOG%"
                        echo     partition table: ^(detected completed dump^)     !TABLE_NAME!
        ) else (
            if exist "%DESTINATION%\!TABLE_NAME!.done" (
                        echo     partition table: ^(detected completed import^)   !TABLE_NAME! >> "%LOG%"
                        echo     partition table: ^(detected completed import^)   !TABLE_NAME!
            ) else (
                echo %DATE% %TIME% > "%DESTINATION%\!TABLE_NAME!.tmp" >nul
                if errorlevel 0 (
                    if exist "%DESTINATION%\!TABLE_NAME!.sql" (
                        echo     partition table: ^(detected completed dump^)  !TABLE_NAME! >> "%LOG%"
                        echo     partition table: ^(detected completed dump^)  !TABLE_NAME!
