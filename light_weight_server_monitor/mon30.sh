...
declare -a mv=(Queries Aborted_clients Aborted_connects Connections Created_tmp_tables Created_tmp_disk_tables Threads_created)
...
function run_memsize {
	...
	( while true ; do
		...
		if [[ -n "${mysqlpid:-}" ]] ; then
			sql="$(awk '$1=="VmHWM:" {peak=$2} $1=="VmRSS:" {rss=$2} $1 == "VmSwap:" {swap=$2} END {printf "%.0fm,%.0fm,%.0fm\n",rss/1024,peak/1024,swap/1024}' /proc/$mysqlpid/status)"
		else
			sql="NO_MYSQL_RUNNING"
		fi
		if [[ -n "${gfpid:-}" ]] ; then
			gf="$(awk '$1=="VmHWM:" {peak=$2} $1=="VmRSS:" {rss=$2} $1 == "VmSwap:" {swap=$2} END {printf "%.0fm,%.0fm,%.0fm\n",rss/1024,peak/1024,swap/1024}' /proc/$gfpid/status)"
		else
			gf="NO_GLASSFISH_RUNNING"
		fi

		echo -e "gf_rss_prss_swap=$gf\tmysql_rss_prss_swap=$sql\t$(date +'%FT%T')" >> $LOGF
	done ) & 
}
...
function one_offs {
	...
	( while true ; do
		...
		{ echo "datetime: $(date +'%FT%T')"
		  ...
		  C=/proc/sys/vm/swappiness; echo "$C: $(cat $C)"
