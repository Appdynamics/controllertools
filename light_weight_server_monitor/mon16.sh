...
function run_dbtest {
	...
	while true ; do
		...
		timeout 59s bash -c 'V=$(HA/mysqlclient.sh -c -t <<< "
...
function run_dbvars {
	...
	while true ; do
		...
		(( $(date +%s) < $DEADLINE )) || exit 0
		timeout 59s bash -c ' 
[[ "$(./HA/mysqlclient.sh -c -t <<< "select '\''YES'\''" 2>&1)" =~ '^YES' ]] || exit 123
...
IFS=, ; echo -e "${vars[*]}\t$(date +'%FT%T')" >> '"$LOGF"
		R=$? 
		(( $R == 123 )) && echo $(date +'%FT%T') '"Can'\''t connect to MySQL server on '\''localhost'\''"' retc=$R >> $LOGF
